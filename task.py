...
def transfer_data(source: str, dest: str, num_row: int):
    ...
    with open(source, "r", encoding="utf-8") as source_file:
        ...
        if (0 < num_row <= len(lines)):
            ...
            print(f"Строка {num_row} из файла {source} успешно скопирована в файл {dest}")
        else:
            print(f"Строки с номером {num_row} не существует в файле {source}")
    pass
...
while True:
    ...
    if mode == 1:
        ...
        num = int(input("Введите номер строки которую нужно скопировать: "))
        transfer_data(file, dest_file, num)
